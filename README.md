...
- **`step14_fgn_stimulus.py`** — fGn-driven A at H ∈ {0.3, 0.5, 0.7, 0.9}. Closed-loop tracking is robust across all H (11–16× improvement vs open-loop, per Block 12 N=100; step 14's original 20–30× was N=1 upper-tail). Prediction-not-tracking is the H-dependent dimension: pred fitness degrades ~4× from H=0.3 to H=0.9, while tracking holds (DISCOVERIES.md X007). The fracnoise lag-1 autocorrelation matches fGn theory at every H.
...
- **`step16_stdp_learning.py`** — First experiment with `plasticity_rate > 0` everywhere. Sweep over plasticity rate from a random initial B pool with closed-loop adrenaline at gain=50. Lifetime STDP does measurable work — at rate=1.0 the post-training fitness improves by **+15.96% ± 0.27% (95% CI, N=100; DISCOVERIES.md X008)**, superseding the original "+14%" single-seed estimate. STDP cannot reach GA-discovered fitness from a random init though — the useful-slot fraction is frozen at initialization because STDP adjusts `v` but not `pre_ids`.
